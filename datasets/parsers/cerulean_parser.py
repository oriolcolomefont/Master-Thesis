...
def process(in_dir, out_dir):
    ...
    for json_file1, json_file2 in zip(annot1_files, annot2_files):
        ...
        create_JAMS(
            ...
            os.path.join(
                out_dir, os.path.basename(json_file1).replace(".json", "") + ".jams"
            ),
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Converts the Cerulean dataset to the JAMS format",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("in_dir", action="store", help="Cerulean main folder")
    parser.add_argument(
        "-o",
        action="store",
        dest="out_dir",
