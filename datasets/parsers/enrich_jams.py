...
def process(in_dir):
    ...
    for audio_file, jam_file in zip(audio_files, jam_files):
        assert (
            os.path.basename(audio_file)[:-4] == os.path.basename(jam_file)[:-5]
        ), "Files names %s and %s must be " "the same." % (
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enriches the JAMS of the Segmentation Dataset by reading "
        "audio information",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("in_dir", action="store", help="Segmentation main folder")
