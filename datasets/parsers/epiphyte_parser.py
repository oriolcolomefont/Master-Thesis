...
def fill_global_metadata(jam, txt_file):
    ...
    jam.metadata.title = os.path.basename(txt_file).replace(".txt", "").split("_")[-1]
    jam.metadata.duration = -1  # In seconds
...
def process(in_dir, out_dir):
    ...
    for i, [segment_file, beat_file] in enumerate(zip(segment_files, beat_files)):
        ...
        create_JAMS(
            ...
            beat_file,
            os.path.join(
                out_dir, os.path.basename(segment_file).replace(".txt", "") + ".jams"
            ),
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Converts the Epiphyte dataset to the JAMS format",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("in_dir", action="store", help="Epiphyte main folder")
    parser.add_argument(
        "-o",
        action="store",
        dest="out_dir",
