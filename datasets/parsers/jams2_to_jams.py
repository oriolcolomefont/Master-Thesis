...
def convert_sections(sections, jam):
    ...
    for section_ann in sections:
        ...
        for context in contexts:
            ...
            ann.annotation_metadata = convert_am(section_ann["annotation_metadata"])
            ...
            for data in section_ann["data"]:
                ...
                # TODO: confidence?
                ann.append(time=data["start"]["value"], duration=dur, value=value)
...
def convert_beats(beats, jam):
    ...
    ann.annotation_metadata = convert_am(beat_ann["annotation_metadata"])

    # Add data
    for data in beat_ann["data"]:
        ann.append(time=data["time"]["value"], duration=0, value=data["label"]["value"])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Converts old jams2 files to the JAMS 0.2 version.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "in_dir", action="store", help="Folder where jams2 files are found"
    )
    parser.add_argument(
        "-o",
        action="store",
