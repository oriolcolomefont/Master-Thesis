...
ISO_ATTRS = {"beat": "beat", "chord": "chordlab", "key": "keylab", "segment": "seglab"}
...
def process(in_dir, out_dir, audio_dir, audio_ext=".wav"):
    ...
    for lab_file in all_labs:
        ...
        if title not in all_jams:
            ...
            output_paths[title] = os.path.join(out_dir, *parts[1:]).replace(
                ".lab", ".jams"
            )
        ...
        jam = all_jams[title]
        tmp_jam, annot = jams.util.import_lab(NS_DICT["segment"], lab_file, jam=jam)
        ...
        # Add Metadata
        curator = jams.Curator(name="Matthias Mauch", email="m.mauch@qmul.ac.uk")
        ann_meta = jams.AnnotationMetadata(
            curator=curator, version=1.0, corpus="Beatles TUT", annotator=None
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("in_dir", action="store", help="Beatles TUT main folder")
    parser.add_argument("out_dir", action="store", help="Output JAMS folder")
    parser.add_argument(
        "-a",
        dest="audio_dir",
        default=None,
